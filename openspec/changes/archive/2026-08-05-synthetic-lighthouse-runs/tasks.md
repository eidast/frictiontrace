...
- [x] 1.10 Add `--throttling-profile <name>` flag with `fast4g` (new default: RTT 60 ms, 9 Mbps down / 1.5 Mbps up, 2x CPU) and `slow4g` (old Lighthouse default, stress test); clear error on unknown profile. Add `throttling_profile TEXT NOT NULL DEFAULT 'slow4g'` column to `synthetic_runs` DDL plus an additive ALTER TABLE migration for existing DBs, and record the profile on each row.
