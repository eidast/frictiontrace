...
The system SHALL provide a script (`npm run crux:group-report`) that reads `data/crux.db` and writes a self-contained HTML file to `reports/crux-group-compare.html` comparing two site cohorts: Walmart (origins in groups `walmart_propios` and `walmart_subsidiarias`) and Competencia (origins in group `otros`).
